...
from EncoderDecoderAgent.ReplayMemory import ReplayMemory, Transition, PrioritizedReplayMemory
...
class BaseTrain:
    def __init__(self, data_loader,
        ...
        self.memory = PrioritizedReplayMemory(ReplayMemorySize)
    ...
    def optimize_model(self):
        ...
        # Get Q values for the current states and actions from the policy network
        state_action_values = self.policy_net(state_batch)
        state_action_values = state_action_values.gather(1, action_batch)

        # Compute V(s_{t+1}) for all next states using the DDQN update
        next_state_values = torch.zeros(self.BATCH_SIZE, device=device)

        # Select the best action for next states using the policy network
        best_actions = self.policy_net(non_final_next_states).max(1)[1].unsqueeze(1)

        # Get the Q value of the best action for next states from the target network
        next_state_values_temp = self.target_net(non_final_next_states)
        next_state_values[non_final_mask] = next_state_values_temp.gather(1, best_actions).squeeze().detach()
        ...
        reg_loss = regularization_factor * reg_loss

        # 结合 Q-Learning 损失和正则化损失
        loss = loss + reg_loss
