...
class Train(BaseTrain):
    def __init__(self,
        ...
        super(Train, self).__init__(data_loader,
                                    ...
                                    TARGET_UPDATE,
                                    n_step)
        # DQN - Trading - master_rainbow_withoutc51
        self.policy_net = Q_NetWork(data_train.state_size, 3).to(device)
        self.target_net = Q_NetWork(data_train.state_size, 3).to(device)
        self.target_net.load_state_dict(self.policy_net.state_dict())
        self.target_net.eval()

        self.optimizer = optim.Adam(self.policy_net.parameters())

        self.test_net = Q_NetWork(self.data_train.state_size, 3)
