...
from DeepRLAgent.VanillaInput.DeepQNetwork import Q_NetWork
from DeepRLAgent.ReplayMemory import ReplayMemory, Transition,PrioritizedReplayMemory
...
class BaseTrain:
    ...
    def optimize_model(self):
        ...
        actions_maximizing_next_q_values = self.policy_net(non_final_next_states).max(1)[1].detach()
        next_state_values = torch.zeros(self.BATCH_SIZE, device=device)
        next_state_values[non_final_mask] = self.target_net(non_final_next_states).gather(1,
                                                                                          actions_maximizing_next_q_values.unsqueeze(
                                                                                              -1)).squeeze().detach()
