...
class SensitivityRun:
    def __init__(self,
                 ...
                 window_size,
                 device,
                 evaluation_parameter='ActionList',
                 transaction_cost=0):
        """
        ...
        self.experiment_path = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                                            'TestResults/' + self.evaluation_parameter + '/')
    ...
    def evaluate_sensitivity(self):
        ...
        for idx in range(key):
            # self.test_portfolios['DQN-pattern'][idx] = self.dqn_pattern.test().get_daily_portfolio_value()
            # self.test_portfolios['DQN-vanilla'][idx] = self.dqn_vanilla.test().get_daily_portfolio_value()
            # self.test_portfolios['DQN-candlerep'][
            #     idx] = self.dqn_candle_rep.test().get_daily_portfolio_value()
            # self.test_portfolios['DQN-windowed'][idx] = self.dqn_windowed.test().get_daily_portfolio_value()
            # self.test_portfolios['MLP-pattern'][idx] = self.mlp_pattern.test().get_daily_portfolio_value()
            # self.test_portfolios['MLP-vanilla'][idx] = self.mlp_vanilla.test().get_daily_portfolio_value()
            # self.test_portfolios['MLP-candlerep'][
            #     idx] = self.mlp_candle_rep.test().get_daily_portfolio_value()
            # self.test_portfolios['MLP-windowed'][idx] = self.mlp_windowed.test().get_daily_portfolio_value()
            # self.test_portfolios['CNN1d'][idx] = self.cnn1d.test().get_daily_portfolio_value()
            # self.test_portfolios['CNN2d'][idx] = self.cnn2d.test().get_daily_portfolio_value()
            self.test_portfolios['GRU'][idx] = self.gru.test().get_daily_portfolio_value()
            # self.test_portfolios['Deep-CNN'][idx] = self.deep_cnn.test().get_daily_portfolio_value()
            # self.test_portfolios['CNN-GRU'][idx] = self.cnn_gru.test().get_daily_portfolio_value()
            # self.test_portfolios['CNN-ATTN'][idx] = self.cnn_attn.test().get_daily_portfolio_value()


    def plot_and_save_sensitivity(self,idx):
        plot_path = os.path.join(self.experiment_path, 'test_graph')
        data_path = os.path.join(self.experiment_path, 'saved_data')
        performance = os.path.join(self.experiment_path, 'performance')
        model = os.path.join(self.experiment_path, 'model')

        # Ensure the necessary directories exist
        for path in [data_path, plot_path, performance,model]:
        ...
        for model_name in self.test_portfolios.keys():
            # print(model_name)
            # Calculate mean profit percentage over all gamma values
            ...
            mean_profit_percentage = [
                ...
                for profits in zip(*all_profit_percentages)
            ]
            # print("before df")
            difference = len(all_profit_percentages[0]) - len(self.data_loader.data_test_with_date)
            df = pd.DataFrame({'date': self.data_loader.data_test_with_date.index,
                               'portfolio': mean_profit_percentage[difference:]})
            df.to_csv(f"{data_path}/{args.dataset_name}_{model_name}_mean_{idx}.csv")

            # Plotting
            ax = df.plot(x='date', y='portfolio', label=model_name)
            ax.set(xlabel='Time', ylabel='%Rate of Return')
            ax.set_title(f'The return rate of {model_name}')
            plt.legend()
            fig_file = os.path.join(plot_path, f'{args.dataset_name}_{model_name}_mean_{idx}.jpg')
    ...
    def save_experiment(self,idx):
        self.plot_and_save_sensitivity(idx)
